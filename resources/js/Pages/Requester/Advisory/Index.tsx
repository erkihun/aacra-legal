...
export default function RequesterAdvisoryIndex({ filters, requests, statusOptions }: AdvisoryIndexProps) {
    const { t, locale } = useI18n();
    const { data, setData } = useForm({ search: filters.search ?? '', status: filters.status ?? '' });

    const applyFilters = () => {
        router.get(route('requester.advisory.index'), data, { preserveState: true, replace: true });
    };

    const resetFilters = () => {
        setData({ search: '', status: '' });
        router.get(route('requester.advisory.index'));
    };
    ...
    return (
        <RequesterLayout
            breadcrumbs={[
                ...
                { label: t('requester.nav_advisory') },
            ]}
        >
            <Head title={t('requester.nav_advisory')} />

            <div className="space-y-6">
                {/* Header */}
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <h1 className="text-xl font-bold text-[color:var(--text)]">{t('requester.advisory_requests')}</h1>
                        <p className="mt-1 text-sm text-[color:var(--muted)]">{t('requester.advisory_index_desc')}</p>
                    </div>
                    <Link href={route('requester.advisory.create')} className="btn-base btn-primary focus-ring">
                        {t('requester.new_advisory')}
                    </Link>
                </div>
                ...
                <div className="overflow-hidden rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)]">
                    {requests.data.length === 0 ? (
                        <div className="px-6 py-16 text-center">
                            <p className="text-sm font-medium text-[color:var(--text)]">{t('requester.no_advisory_yet')}</p>
                            <p className="mt-1 text-xs text-[color:var(--muted)]">{t('requester.advisory_empty_hint')}</p>
                        </div>
                    ) : (
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-[color:var(--border)] text-left text-xs font-semibold uppercase text-[color:var(--muted)]">
                                    <th className="px-5 py-3">{t('requester.request_number')}</th>
                                    <th className="px-5 py-3">{t('requester.category')}</th>
                            ...
                            <tbody className="divide-y divide-[color:var(--border)]">
                                {requests.data.map((item) => (
                                    <tr key={item.id} className="hover:bg-[color:var(--surface-muted)]">
                                        <td className="px-5 py-3 font-mono text-xs text-[color:var(--muted)]">{item.request_number}</td>
                                        <td className="px-5 py-3 text-[color:var(--text)]">
                                            {(locale === 'am' ? item.category?.name_am : null) ?? item.category?.name_en ?? '—'}
                                        </td>
                                        ...
                                        <td className="px-5 py-3 text-right">
                                            <Link
                                                href={route('requester.advisory.show', { advisoryRequest: item.id })}
