...
export default function HeroSlider({
    ...
    return (
        <section className="relative overflow-hidden rounded-[2rem] border border-[color:var(--border)] bg-[color:var(--surface-strong)] shadow-[var(--shadow-strong)]">
            ...
            <div className="relative min-h-[32rem] sm:min-h-[35rem] lg:min-h-[40rem]">
                {safeSlides.map((slide, index) => (
                    ...
                    >
                        ...
                        <div className="relative flex h-full flex-col px-5 py-6 sm:px-8 sm:py-8 lg:px-12 lg:py-12">
                            <div className="max-w-4xl">
                                ...
                                <h1 className="mt-5 max-w-3xl text-3xl font-semibold leading-tight tracking-tight text-white sm:mt-6 sm:text-5xl lg:text-6xl">
                                    {slide.title}
                                </h1>
                                ...
                                <div className="mt-6 flex flex-col gap-3 sm:mt-8 sm:flex-row sm:flex-wrap">
                                    {slide.button_label && slide.button_url ? (
                                        <Link href={slide.button_url} className="btn-base w-full bg-white text-slate-900 shadow-[0_18px_40px_-20px_rgba(15,23,42,0.55)] hover:bg-slate-100 focus-ring sm:w-auto">
                                            {slide.button_label}
                                        </Link>
                                    ) : null}
                                    <Link href={primaryCtaHref} className="btn-base w-full border border-white/15 bg-white/10 text-white backdrop-blur-md hover:bg-white/15 focus-ring sm:w-auto">
                                        {primaryCtaLabel}
                                    </Link>
                                    <Link href={secondaryCtaHref} className="btn-base w-full border border-white/15 bg-transparent text-white hover:bg-white/10 focus-ring sm:w-auto">
                                        {secondaryCtaLabel}
                                    </Link>
                                    <Link href={tertiaryCtaHref} className="btn-base w-full border border-white/10 bg-transparent text-white/80 hover:bg-white/10 hover:text-white focus-ring sm:w-auto">
                                        {tertiaryCtaLabel}
                                    </Link>
                                </div>
                            </div>
                                    ...
                                    <div className="grid gap-3 sm:grid-cols-3 lg:max-w-3xl lg:flex-1">
                                        {metrics.map((metric) => (
                                            <div key={metric.label} className="rounded-[1.4rem] border border-white/12 bg-white/10 px-4 py-4 backdrop-blur-md">
                                                ...
                                                <p className="mt-2 text-xl font-semibold text-white sm:text-2xl">{metric.value}</p>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    {safeSlides.length > 1 ? (
                                        <div className="flex items-center justify-between gap-4 sm:justify-end">
                                            <div className="flex items-center gap-2">
                                                {safeSlides.map((slide, indicatorIndex) => (
                                                    <button
                                                        key={`${slide.title}-indicator-${indicatorIndex}`}
                                                        type="button"
                                                        aria-label={`${indicatorIndex + 1}`}
                                                        onClick={() => setActiveIndex(indicatorIndex)}
                                                        className={`h-2.5 rounded-full transition-all ${indicatorIndex === activeIndex ? 'w-10 bg-white' : 'w-2.5 bg-white/45 hover:bg-white/70'}`}
                                                    />
                                                ))}
                                            </div>
